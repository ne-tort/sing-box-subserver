...
echo '== status =='
curl -fsS -H "$H" http://127.0.0.1:8080/v1/status | python3 -m json.tool | head -40
echo '== tls =='
curl -fsS -H "$H" http://127.0.0.1:8080/v1/controlplane/tls | python3 -m json.tool | head -40
echo '== config snippet =='
curl -fsS -H "$H" http://127.0.0.1:8080/v1/config | python3 -c 'import json,sys; d=json.load(sys.stdin); print("providers", d.get("certificate_providers"));
ibs=d.get("inbounds",[]);
...
curl -fsS -X POST -H "$H" http://127.0.0.1:8080/v1/controlplane/sets/tr1/activate || true
curl -fsS -X PUT -H "$H" -H "Content-Type: application/json" \
  ...
  http://127.0.0.1:8080/v1/controlplane/tls >/dev/null
curl -fsS -H "$H" http://127.0.0.1:8080/v1/config | python3 -c 'import json,sys; d=json.load(sys.stdin); print("providers", d.get("certificate_providers"));
