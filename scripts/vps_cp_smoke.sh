...
echo "== health =="
curl -fsS "$BASE/v1/health" | grep -q alive || fail health
pass health

echo "== version =="
VER=$(curl -fsS -H "Authorization: Bearer ${TOKEN}" "$BASE/v1/version")
echo "$VER" | head -c 400; echo
echo "$VER" | grep -q with_controlplane || note "build_tags missing with_controlplane (version package gap)"

echo "== status =="
ST=$(curl -fsS -H "Authorization: Bearer ${TOKEN}" "$BASE/v1/status")
...
echo "== user + ss + trojan =="
USER_JSON=$(curl -fsS -X POST "${AUTH[@]}" "$BASE/v1/controlplane/users" -d '{"name":"alice"}')
TOK=$(python3 -c 'import json,sys; print(json.load(sys.stdin)["data"]["sub_token"])' <<<"$USER_JSON")
test -n "$TOK" || fail sub_token
curl -fsS -X POST "${AUTH[@]}" "$BASE/v1/controlplane/sets" \
...
ACT2=$(curl -fsS -X POST -H "Authorization: Bearer ${TOKEN}" "$BASE/v1/controlplane/sets/tr1/activate")
...
curl -fsS --connect-timeout 5 "http://${HOST_PUBLIC}:8080/v1/health" | grep -q alive \
  || fail public health
pass public health
