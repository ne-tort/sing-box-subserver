#!/usr/bin/env python3
"""One-shot naive demux debug with box.log dump."""
...
wd = HERE / "work" / "_probe_naive" / "demux_debug"
...
info = render_group_workdir(
    ...
    lx_bin=lx,
    image=IMAGE,
    slot_presets={"tls": "naive_tls"},
)
tag = "naive_tls" if "naive_tls" in [c["tag"] for c in info["cells"]] else info["cells"][0]["tag"]
...
rewrite_client_iperf_ip(wd, tag, iperf_ip, server_ip=server_ip)

cname = "naiveprobe-demux-dbg"
