...
echo "== catalogs =="
PRESETS=$(curl -fsS -H "$H" "$BASE/v1/controlplane/presets")
...
RECIPES=$(curl -fsS -H "$H" "$BASE/v1/controlplane/demux-recipes")
...
curl -fsS -H "$H" "$BASE/v1/controlplane/demux-recipes/tls-trojan-plain-vless" | python3 -c 'import json,sys; r=json.load(sys.stdin)["data"]; assert "demux_template" in r'
ST=$(curl -fsS -H "$H" "$BASE/v1/controlplane/status")
...
curl -fsS -X POST -H "$H" -H "$CT" -d '{"name":"acme-http01"}' "$BASE/v1/controlplane/users" >/tmp/u.json
curl -fsS -X POST -H "$H" -H "$CT" \
  -d '{"name":"trojan-443","listen":"::","listen_port":443,"presets":["trojan-tcp"]}' \
  "$BASE/v1/controlplane/sets" >/dev/null
curl -fsS -X POST -H "$H" "$BASE/v1/controlplane/sets/trojan-443/activate" >/dev/null || true
pass set_ready
...
curl -fsS -X PUT -H "$H" -H "$CT" -d "{\"mode\":\"acme_domain\",\"acme\":{\"email\":\"${EMAIL}\",\"domains\":[\"${DOMAIN}\"],\"provider\":\"letsencrypt\"}}" \
...
curl -fsS -X POST -H "$H" "$BASE/v1/controlplane/sets/trojan-443/activate" >/tmp/act.json || { cat /tmp/act.json; docker logs subserver-cp 2>&1 | tail -40; fail activate; }
sleep 8
ok=0
...
curl -fsS -X PUT -H "$H" -H "$CT" -d "{\"mode\":\"acme_domain\",\"acme\":{\"email\":\"${EMAIL}\",\"domains\":[\"${DOMAIN}\"],\"provider\":\"letsencrypt\",\"disable_tls_alpn_challenge\":true,\"alternative_http_port\":${ALT}}}" \
  "$BASE/v1/controlplane/tls" >/dev/null
curl -fsS -X POST -H "$H" "$BASE/v1/controlplane/sets/trojan-443/activate" >/tmp/act2.json || { cat /tmp/act2.json; docker logs subserver-cp 2>&1 | tail -80; fail alt_activate; }
sleep 10
ok=0
