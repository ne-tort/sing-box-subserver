...
func (s *Service) handleClientBootstrap(w http.ResponseWriter, r *http.Request) {
	...
	okJSON(w, 200, map[string]any{
		...
		"capabilities": map[string]any{
			...
			"tls_modes":            []string{"self_signed", "acme_domain", "acme_ip"},
			"demux_action":         "dial",
			"optional_listen_port": true, // omit listen_port → auto-pick free port (presets + demux)
			"ready_poll":           true,
			"wg_hub":               true,
			"activate_contract":    "activated is always boolean; activated_sets lists set names for from-presets",
		},
		...
		"flows": []map[string]any{
			{
				...
				"steps": []map[string]any{
					...
					{"method": "POST", "path": "/v1/controlplane/sets/from-presets", "why": "install; listen_port optional (auto); activate:true", "check": "response.activated === true"},
			...
			},
			{
				...
				"steps": []map[string]any{
					...
					{"method": "POST", "path": "/v1/controlplane/sets/from-demux-group", "why": "install + activate; listen_port optional", "check": "response.activated === true"},
					{"method": "GET", "path": "/v1/controlplane/status", "why": "poll until ready.ok === true"},
					{"method": "POST", "path": "/v1/controlplane/users", "why": "create client + subscription_url"},
