...
func BuildInstall(req InstallRequest, usedPorts map[uint16]struct{}) (InstallResult, error) {
	...
	snis, warn := assignSlotSNIs(g, slotPreset)
	...
	presets := make([]string, 0, len(g.Slots))
	for _, slot := range g.Slots {
		pn := slotPreset[slot.ID]
		presets = append(presets, pn)
...
func assignSlotSNIs(g Group, slotPreset map[string]string) (map[string]string, []string) {
...
func Substitutions(tag string) (SubstitutionsView, error) {
	...
	for _, s := range g.Slots {
		...
		for _, pn := range presetsList {
			opt := PresetOption{Tag: pn, DemuxCompat: demuxCompatForPreset(pn, s.Role, s.MatchHint)}
